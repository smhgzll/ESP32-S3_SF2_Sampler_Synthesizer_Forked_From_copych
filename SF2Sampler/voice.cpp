...
/*
void Voice::start(uint8_t channel_, uint8_t note_, uint8_t velocity_, Zone zone_, ChannelState* ch) {

    zone = std::move(zone_);
    SampleHeader* s_ = zone_.sample;
    samplesRun = lastSamplesRun = 0;
    ...
    // Determine portamento and mono mode behavior  
    portamentoActive = false;
    if (modPortamento && *modPortamento && ch->monoMode != Poly) {
        portamentoActive = true;
        
        float noteDiff = float(note_ - startNote);           // semitone difference
